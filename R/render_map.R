...
render_cis_map <- function(results_enriched, out_path) {
  ...
  crear_zoom <- function(codigo_prov, nombre_prov, n_cols_grid, spacing_custom,
    ...
    p <- p + geom_text(aes(x = cx_local, y = y_texto, label = nombre_prov), fontface = "bold", size = 3.5, vjust = 0) +
  ...
  plot_resumen_nacional <- ggplot() +
    ...
    geom_text(aes(x = 1.0, y = 0.02), label = "Seats", color = "black", size = 3, fontface = "bold") +
    geom_text(aes(x = 2.9, y = 0.02), label = "Votes", color = "black", size = 3, fontface = "bold") +
  ...
  mapa_base <- ggplot() +
    ...
    geom_text(data = labels_shift,
              aes(x = cx, y = y_label, label = provincia_limpia,
                  color = if_else(is_displaced, "black", if_else(final_hex == HEX_PP_OSCURO, "white", "black", missing = "black"))),
  ...
  crear_bloque_leyenda <- function(datos, mostrar_headers = FALSE) {
    ...
    if (mostrar_headers) {
      ...
      p <- p + geom_text(data = headers, aes(x = x, y = y, label = label), color = "black", fontface = "bold", size = 3, hjust = 0.5)
    }
    return(p)
  }
