# analysis/first-steps/exploratory_csslab.R
#
# EXPLORATORY SCRIPT — NOT PART OF THE PIPELINE
#
# Replicates the CSSLab cartographic composition (CSSLab UC3M data viz project,
# 2023 Spanish general election map) but feeds it with vote shares and seat
# allocations *projected directly* from the CIS barometer 3557 (April 2026).
#
# Methodologically weak by design (direct provincial tabulation from a national
# survey, very thin provincial sub-samples).
#
# The strategy is to build a `results_enriched` tibble shaped exactly like the
# one the CSSLab Rmd produces from the official 2023 Excel, but with the
# projected votes and projected seats coming from the survey. From there
# downstream, all visual code is taken verbatim from the CSSLab project.
#
# Output: outputs/first-outputs/exploratory_map_3557_csslab_style.png
...
source("R/dhondt.R")
dir.create("outputs/first-outputs", recursive = TRUE, showWarnings = FALSE)
...
# ---------------------------------------------------------------------------
# 2. Project vote shares per province from CIS 3557
# ---------------------------------------------------------------------------
sav_path <- "data-raw/cis/3557.sav"
stopifnot(file.exists(sav_path))

raw_cis <- haven::read_sav(sav_path)
...
# Map CIS codes to the CSSLab party suffixes
...
# Save
out_path <- "outputs/first-outputs/exploratory_map_3557_csslab_style.png"
